...
![Skematik Kipas Otomatis](Skematik_Kipas_Otomatis.png)
...
```
📦 kipas-otomatis-arduino/
├── 📄 README.md
├── 📂 src/
│   └── kipas_otomatis.ino       # Source code utama
├── 📂 docs/
│   ├── Skematik_Kipas_Otomatis.png
│   └── Laporan_Project.pdf
└── 📂 simulation/
    └── link_tinkercad.txt       # Link simulasi Tinkercad
```
...
1. **Clone repository ini:**
   ```bash
   git clone https://github.com/username/kipas-otomatis-arduino.git
   ```

2. **Buka file** `src/kipas_otomatis.ino` di **Arduino IDE**.

3. **Install library yang dibutuhkan** (via Library Manager):
   - `LiquidCrystal_I2C`
   - `Wire` (sudah built-in)

4. **Rangkai hardware** sesuai wiring diagram di atas.

5. **Upload sketch** ke Arduino Uno (Board: Arduino Uno, Port: sesuaikan).

6. **Buka Serial Monitor** (baud 9600) untuk debug, atau amati langsung di LCD.
